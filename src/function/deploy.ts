...
import { ContractType } from "../utils.ts";
...
export default async function deploy(
  ...
  if (contract.tx) {
    ...
    const tx = await arweave.createTransaction({
      data: new TextEncoder().encode(initState),
    });
    ...
    console.log("\nInit Contract:", {
      id: tx.id,
      tags: tx.tags.map(({ name, value }: { [x: string]: string }) => ({
        name: atob(name),
        value: atob(value),
      })),
      data: new TextDecoder().decode(tx.data),
    });
    ...
    const tx1 = await arweave.createTransaction({
      data: new TextEncoder().encode(contract.src),
    });
    ...
    console.log("\nSource Contract:", {
      ...
      data: new TextDecoder().decode(tx1.data),
    });
    ...
    console.log("\nInit Contract:", {
      ...
      data: new TextDecoder().decode(tx2.data),
    });
